...
mod tests {
    ...
    fn test_hsl_functions() {
        ...
        let css = compiler
            .compile(".test { color: hsl(0, 100%, 50%); }")
            .unwrap();
        assert!(
            css.contains("#ff0000") || css.contains("#f00"),
            "hsl(0, 100%, 50%) should be red, got: {}",
            css
        );

        // Green is hue 120, saturation 100%, lightness 50%
        let css = compiler
            .compile(".test { color: hsl(120, 100%, 50%); }")
            .unwrap();
        assert!(
            css.contains("#00ff00") || css.contains("#0f0"),
            "hsl(120, 100%, 50%) should be green, got: {}",
            css
        );

        // HSLA
        let css = compiler
            .compile(".test { color: hsla(240, 100%, 50%, 0.5); }")
            .unwrap();
        assert!(
            css.contains("rgba(0, 0, 255, 0.5)"),
            "hsla(240, 100%, 50%, 0.5) failed, got: {}",
            css
        );
    }
    ...
    fn test_saturation_functions() {
        ...
        let css = compiler
            .compile(".test { color: desaturate(#ff0000, 100%); }")
            .unwrap();
        assert!(
            css.contains("#808080"),
            "desaturate(#ff0000, 100%) failed, got: {}",
            css
        );
    ...
    fn test_fade_functions() {
        let mut compiler = Compiler::new();
        let css = compiler
            .compile(".test { color: fade(#ff0000, 50%); }")
            .unwrap();
        assert!(
        ...
        let css = compiler
            .compile(".test { color: fadeout(#ff0000, 10%); }")
            .unwrap();
        // #ff0000 is alpha 1.0 -> fadeout 10% -> 0.9
        assert!(
            css.contains("rgba(255, 0, 0, 0.9)"),
            "fadeout failed, got: {}",
            css
        );
    ...
    fn test_spin_function() {
        ...
        let css = compiler
            .compile(".test { color: spin(#ff0000, 180); }")
            .unwrap();
        assert!(
            css.contains("#00ffff") || css.contains("#0ff"),
            "spin failed, got: {}",
            css
        );
    }
    ...
    fn test_mix_function() {
        ...
        let css = compiler
            .compile(".test { color: mix(#ff0000, #0000ff); }")
            .unwrap();
        // Red: 255, 0, 0
        // Blue: 0, 0, 255
